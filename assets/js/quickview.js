...
function facebPopUp(URL) {
    day = new Date();
    id = day.getTime();
    eval("page" + id + " = window.open(URL, '" + id + "', 'toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=0,width=640,height=480,left = 20,top = 22');");
    return false;
}
...
function cl(el, ct, ps) {
    if (document.images) {
        (new Image()).src = "url?ct=" + escape(ct) + "&ps=" + escape(ps) + "&url=" + escape(el.href);
    }
    return true;
}
...
function clb(u, a, t, p, n) {
    if (document.images) {
        (new Image()).src = "urlredir?a=" + escape(a) + "&t=" + escape(t) + "&p=" + escape(p) + "&n=" + escape(n) + "&u=" + escape(u.href) + "&q=" + escape('avatars');
    }
    return true;
}
